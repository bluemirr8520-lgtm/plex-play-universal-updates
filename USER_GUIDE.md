...
1. GitHub 최신 릴리스에서 `PlexPlayUniversal.apk`를 내려받습니다.
...
실패를 기다리지 않고 처음부터 범용 코덱으로 재생합니다. 초록색 원본 지원 영상은 기존
재생기를 우선하며, 사용자가 1080p·720p·480p 품질을 선택한 경우에는 Plex 변환 스트림을
우선합니다.

범용 화면의 톱니바퀴는 기본 재생과 같은 메인 메뉴와 하위 메뉴 구조로 열립니다.
...
호환 재생은 기기 호환성이 높은 SurfaceView 출력을 사용합니다. 화면 밝기와 화면 크기는
즉시 적용되며, 일부 OTT 기기에서는 고급 색 조정의 적용 범위가 제한될 수 있습니다.
...
PGS/VobSub 같은 그림 자막은 영상에 포함된 원래 모양을 사용하므로 글꼴 설정이 적용되지
않습니다. 텍스트 자막의 글꼴·크기·색상·외곽선·배경·위치·세로쓰기 설정은 선택 즉시
...
슬라이더는 리모컨 방향키 또는 터치로 조절합니다. 좌우 방향키는 값만 조절하고,
위아래 방향키는 이전·다음 설정 항목으로 이동합니다. 자막 위치의 가로·세로 슬라이더도
같은 방식이라 슬라이더에 포커스가 갇히지 않습니다. 조절한 값은 사용자 설정 화면
모드로 저장됩니다.
...
- 계속 종료되면 기기를 재부팅한 뒤 1.0.8 APK를 다시 설치하고 같은 영상을 확인합니다.
